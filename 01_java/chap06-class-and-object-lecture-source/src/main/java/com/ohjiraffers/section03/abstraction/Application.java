...
public class Application {
    public static void main(String[] args) {
        /*자바는 객체지향 프로그래밍 언어
        * 이를 이해하고 객체와 클래스를 설계하여 프로그래밍을 할 수 있어야 한다.
        * 문은 사물.
        * 문: 열린다, 닫힌다 -> 기능
        * 두 개의 객체가 상호작용.
        * 메세지를 통해 호출(문을 닫아줘, 열어줘 라는 메세지). 그것이 메소드.
        * 클래스와 클래스, 객체와 객체의 연결고리가 메소드이다.
        *
        * 객체지향 프로그래밍이란
        * 현실세계의 모든 사건(event)는 객체와 객체의 상호작용에 의해 일어난다는 세계관을
        * 프로그래밍으로 만들 때 이용하여 새로운 세계를 창조하는 방법론이다.
        * 현실세계와 닮아있지만 현실세계와 다른점은
        * 객체간의 상호작용을 한다는 사실은 현실세계와 동일하지만 의인화 기법이 적용되어
        * 현실세계에서는 불가능한 무생물이나 개념같은 존재들도
        * 하나의 주체로 본인의 상태를 스스로 제어하고 행동하도록 한다는 것이
        * 현실세계와는 다른 점이다(캡슐화의 의미).
        ...
        *
        * 추상화 : 미술 작품에 쓰이는 기법.
        * 우리가 "사과다." 했을 때 떠오르는 이미지 : 색, 모양, 크기가 떠오르며 다른 과일과는 차별하여 생각이 가능하다.
        * 식탁이다 라고 했을 때도 모양이 비슷하게 떠오를 것이다.
        * */

        /*객체와 객체의 상호작용이란?
        * 객체와 객체는 메세지를 통해 서로 상호작용을 한다. 메세지(메소드 호출)를 보내는 쪽을
        * 송신자라고 하고, 메세지를 수신하는 쪽을 수신자라고 한다.
        ...
        * 따라서 객체를 설계할 때는 속성은 부가적인이며 적절한 책임을 가진 객체들간의 주고 받는 메세지를 이용해
        * 상호작용하는 것을 설계하는 것을 의미한다.
        *
        * 필드보다는 메소드를 중점으로 추상화기법을 적용하여 객체를 설계하고 그 객체를 구현하는 방법으로
        * 설계한대로 클래스를 작성하는 것이다. 이것을 추상화라고 부른다.
        * */
        ...
        * 플레이어(사용자), 카레이서, 자동차
        * 2. 객체 간 상호작용
        * - 카레이서가 수신할 수 있는 메세지(요청을 받았을 때 카레이서가 해야할 일-플레이어가 카레이서에게 요청)
        * 1. 시동을 걸어라
        * 2. 액셀레이터를 밟아라
        * 3. 브레이크를 밟아라
        * 4. 시동을 꺼라
